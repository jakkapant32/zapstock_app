...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await db.query(
      'SELECT u.id, u.username, u.password_hash, u.role, up.full_name, up.email FROM users u LEFT JOIN user_profiles up ON u.id = up.user_id WHERE u.username = $1',
      [username]
    );
...
router.get('/verify', async (req, res) => {
  try {
    ...
    const session = await db.query(
      'SELECT s.user_id, s.expires_at, u.username, u.role, up.full_name, up.email FROM sessions s JOIN users u ON s.user_id = u.id LEFT JOIN user_profiles up ON u.id = up.user_id WHERE s.token_hash = $1 AND s.is_active = true AND s.expires_at > NOW()',
      [token]
    );
