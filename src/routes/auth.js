...
router.put('/change-password', async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO activity_logs (user_id, action, description, table_name) VALUES ($1, $2, $3, $4)',
      [userId, 'เปลี่ยนรหัสผ่าน', 'ผู้ใช้เปลี่ยนรหัสผ่านสำเร็จ', 'users']
    );
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    // ค้นหาผู้ใช้จากอีเมล
    const user = await db.query(
      'SELECT u.id, u.username, up.email FROM users u JOIN user_profiles up ON u.id = up.user_id WHERE up.email = $1',
      [email]
    );

    if (user.rows.length === 0) {
      // ไม่แสดงข้อผิดพลาดเพื่อป้องกันการสแกนอีเมล
      return res.json({
        success: true,
    ...
    // บันทึก reset token (ใช้ตาราง sessions ชั่วคราว)
    await db.query(
      'INSERT INTO sessions (user_id, token_hash, expires_at) VALUES ($1, $2, $3)',
      [user.rows[0].id, `reset_${resetToken}`, expiresAt]
    );

    // บันทึก activity log
    await db.query(
      'INSERT INTO activity_logs (user_id, action, description, table_name) VALUES ($1, $2, $3, $4)',
      [user.rows[0].id, 'ขอรีเซ็ตรหัสผ่าน', `ขอรีเซ็ตรหัสผ่านสำหรับอีเมล: ${email}`, 'users']
    );

    // TODO: ส่งอีเมลจริง (ตอนนี้แค่ return token สำหรับการทดสอบ)
    res.json({
      success: true,
      message: 'หากอีเมลนี้มีอยู่ในระบบ เราจะส่งลิงก์รีเซ็ตรหัสผ่านให้',
      resetToken: resetToken // เอา token ออกใน production
    });
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    // ตรวจสอบ reset token
    const session = await db.query(
      'SELECT s.user_id FROM sessions s WHERE s.token_hash = $1 AND s.is_active = true AND s.expires_at > NOW()',
      [`reset_${resetToken}`]
    );
    ...
    await db.query(
      'INSERT INTO activity_logs (user_id, action, description, table_name) VALUES ($1, $2, $3, $4)',
      [userId, 'รีเซ็ตรหัสผ่าน', 'รีเซ็ตรหัสผ่านสำเร็จ', 'users']
    );
