...
export default function PoorSellingProducts() {
  ...
  const getPoorSellingReason = (currentStock: number, minStock: number) => {
    if (currentStock === 0) return { text: 'หมดสต็อก', color: '#FF3B30', bgColor: '#FFE5E5' };
    if (currentStock <= minStock) return { text: 'สต็อกต่ำ', color: '#FF9500', bgColor: '#FFF5E5' };
    if (currentStock <= minStock * 2) return { text: 'ขายช้า', color: '#FFD700', bgColor: '#FFFDE5' };
    return { text: 'ปกติ', color: '#34C759', bgColor: '#E5F5E5' };
  };

  const poorProducts = getPoorSellingProducts();
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>สินค้าขายไม่ดี</Text>
      ...
      >
        ...
        <View style={styles.warningBanner}>
          ...
          <Text style={styles.warningText}>
            สินค้าเหล่านี้มีสต็อกสูงและขายช้า ควรพิจารณาปรับกลยุทธ์การขาย
          </Text>
        </View>
        ...
        <View style={styles.productsContainer}>
          {poorProducts.length > 0 ? (
            poorProducts.map((product, index) => {
              const stockStatus = getStockStatus(product.currentStock, product.minStockQuantity);
              const poorReason = getPoorSellingReason(product.currentStock, product.minStockQuantity);
              return (
                <View key={product.id} style={styles.productCard}>
                  <View style={styles.productImageContainer}>
                    <Image source={{ uri: product.image }} style={styles.productImage} />
                    <View style={styles.poorRankBadge}>
                      <Text style={styles.poorRankText}>#{index + 1}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.productInfo}>
                    ...
                    <View style={styles.productStats}>
                      ...
                      <View style={styles.statItem}>
                        <Ionicons name="trending-down" size={16} color="#8E8E93" />
                        <Text style={styles.statText}>
                    ...
                    <View style={styles.statusContainer}>
                      ...
                      <View style={[styles.poorReasonBadge, { backgroundColor: poorReason.bgColor }]}>
                        <Text style={[styles.poorReasonText, { color: poorReason.color }]}>
                          {poorReason.text}
                        </Text>
                      </View>
                    </View>
                  ...
                  <View style={styles.productActions}>
                    ...
                    >
                      <Ionicons name="megaphone" size={20} color="#FF3B30" />
                      <Text style={styles.promoteButtonText}>โปรโมท</Text>
                    </TouchableOpacity>
            ...
            <View style={styles.emptyState}>
              <Ionicons name="trending-down" size={64} color="#C7C7CC" />
              <Text style={styles.emptyStateText}>ยังไม่มีสินค้าขายไม่ดี</Text>
              <TouchableOpacity 
